...
function get_all_tasks_due_today($conn){
	$sql = "SELECT * FROM tasks WHERE due_date = CURDATE() AND status != 'completed' ORDER BY assigned_to DESC";
	$stmt = $conn->prepare($sql);
	$stmt->execute([]);
...
function get_all_tasks_noDeadline($conn){
	$sql = "SELECT * FROM tasks WHERE status != 'completed' AND due_date IS NULL OR due_date = '0000-00-00' ORDER BY assigned_to DESC";
	$stmt = $conn->prepare($sql);
	$stmt->execute([]);
